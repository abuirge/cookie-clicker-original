...
Game.Launch = function () {
  ...
  Game.Init = function () {
    ...
    l("javascriptError").innerHTML =
      '<div style="padding:64px 128px;"><div class="title">Loading...</div></div>';
    ...
    l("links").innerHTML =
      '<a href="http://orteil.dashnet.org/experiments/cookie/" target="blank">Cookie Clicker Classic</a>';
    ...
    Game.ExportSave = function () {
      var save = prompt(
        "Copy this text and keep it somewhere safe!",
        Game.WriteSave(1)
      );
    };
    ...
    Game.WriteSave = function (
      exporting //guess what we'e using to save the game?
    ) {
      var str = "";
      str += Game.version + "|";
      str += "|"; //just in case we need some more stuff here
      str += parseInt(Game.startDate) + "|"; //save stats
      str += //prefs
        ...
        "|";
      str +=
        parseFloat(Math.floor(Game.cookies)) +
        ";" +
      ...
      if (exporting) {
        ...
        if (document.cookie.indexOf("CookieClickerGame") < 0)
    ...
    Game.LoadSave = function (data) {
      ...
      else if (document.cookie.indexOf("CookieClickerGame") >= 0)
        str = unescape(document.cookie.split("CookieClickerGame=")[1]); //get cookie here
      ...
      if (str != "") {
        ...
        if (str != "") {
          ...
          if (version >= 1 && version > Game.version) {
            alert(
              "Error : you are attempting to load a save from a later version (v." +
                version +
                "; you are using v." +
                Game.version +
                ")."
          ...
          Game.Upgrades["Elder Pledge"].basePrice = Math.pow(
            8,
            Math.min(Game.pledges + 2, 13)
          );
    ...
    Game.HardReset = function () {
      if (
        ...
        if (
          confirm(
            "Whoah now, are you really, REALLY sure?\n(don't say we didn't warn you!)"
          )
        ) {
          for (var i in Game.AchievementsById) {
            var me = Game.AchievementsById[i];
    ...
    Game.mouseCps = function () {
      ...
      return (
        ...
        Game.ComputeCps(
          ...
          Game.Has("Carpal tunnel prevention cream") + Game.Has("Ambidextrous"),
          add
        )
      );
    };
    ...
    Game.HowMuchPrestige = function (cookies) {
      ...
      prestige = Math.max(
        0,
        Math.floor((-1 + Math.pow(1 + 8 * prestige, 0.5)) / 2)
    ...
    Game.CalculateGains = function () {
      ...
      for (var i = 0; i < Game.cpsAchievs.length / 2; i++) {
        if (Game.cookiesPs >= Game.cpsAchievs[i * 2 + 1])
          Game.Win(Game.cpsAchievs[i * 2]);
      }
    ...
    Game.goldenCookie.spawn = function () {
      if (Game.goldenCookie.delay != 0 || Game.goldenCookie.life != 0)
        Game.Win("Cheated cookies taste awful");
      var me = l("goldenCookie");
      if (
        (Game.elderWrath == 1 && Math.random() < 0.33) ||
        (Game.elderWrath == 2 && Math.random() < 0.66) ||
        Game.elderWrath == 3
      ) {
      ...
      Game.goldenCookie.x =
        Math.floor(
          Math.random() * (screen.right - screen.left - 128) + screen.left + 64
        ) - 64;
      Game.goldenCookie.y =
        Math.floor(
          Math.random() * (screen.bottom - screen.top - 128) + screen.top + 64
        ) - 64;
    ...
    Game.goldenCookie.update = function () {
      ...
      if (Game.goldenCookie.life > 0) {
        ...
        l("goldenCookie").style.opacity =
          1 -
          Math.pow(
            (Game.goldenCookie.life / (Game.fps * Game.goldenCookie.dur)) * 2 -
              1,
            4
          );
    ...
    Game.goldenCookie.choose = function () {
      var list = [];
      if (Game.goldenCookie.wrath > 0)
        list.push("clot", "multiply cookies", "ruin cookies");
      else list.push("frenzy", "multiply cookies");
      if (Game.goldenCookie.wrath > 0 && Math.random() < 0.3)
        list.push("blood frenzy", "chain cookie");
      else if (Math.random() < 0.01 && Game.cookiesEarned >= 100000)
        list.push("chain cookie");
      if (Math.random() < 0.05) list.push("click frenzy");
      if (
        Game.goldenCookie.last != "" &&
        Math.random() < 0.8 &&
        list.indexOf(Game.goldenCookie.last) != -1
      )
        list.splice(list.indexOf(Game.goldenCookie.last), 1); //80% chance to force a different one
    ...
    Game.goldenCookie.click = function () {
      if (Game.goldenCookie.life > 0) {
        ...
        if (choice == "frenzy") {
          ...
          var moni =
            Math.min(Game.cookies * 0.1, Game.cookiesPs * 60 * 20) + 13; //add 10% to cookies owned (+13), or 20 minutes of cookie production - whichever is lowest
          Game.Earn(moni);
          ...
          Game.recalculateGains = 1;
          Game.Popup(
            "Elder frenzy : cookie production x666 for " + time + " seconds!"
          );
        } else if (choice == "clot") {
          ...
          Game.recalculateGains = 1;
          Game.Popup(
            "Clot : cookie production halved for " + time + " seconds!"
          );
          ...
          Game.recalculateGains = 1;
          Game.Popup(
            "Click frenzy! Clicking power x777 for " + time + " seconds!"
          );
        } else if (choice == "chain cookie") {
          Game.goldenCookie.chain++;
          var moni = "";
          for (var i = 0; i < Game.goldenCookie.chain; i++) {
            moni += "6";
          }
          moni = parseInt(moni);
          ...
          if (
            (Math.random() < 0.1 ||
              Game.goldenCookie.chain > 12 ||
              moni >= Game.cookies * 1) &&
            Game.goldenCookie.chain > 4
          )
            Game.goldenCookie.chain = 0;
          Game.Earn(moni);
        }
    ...
    Game.cookieParticleAdd = function () {
      ...
      if (Game.prefs.particles) {
        ...
        me.l.style.backgroundPosition =
          Math.floor(Math.random() * 8) * 64 + "px 0px";
    ...
    for (var i = 0; i < 20; i++) {
      ...
      str +=
        '<div id="cookieNumber' + i + '" class="cookieNumber title"></div>';
    }
    ...
    Game.updateClasses = function () {
      var str = "";
      for (var i in Game.cssClasses) {
        str += Game.cssClasses[i] + " ";
      }
    ...
    Game.UpdateMenu = function () {
      ...
      if (Game.onMenu != "") {
        str +=
          '<div style="position:absolute;top:8px;right:8px;cursor:pointer;font-size:16px;" onclick="Game.ShowMenu(Game.onMenu);">X</div>';
      }
      if (Game.onMenu == "prefs") {
        str +=
          ...
          "<label>(note : this disables update notifications)</label></div>" +
      ...
      if (Game.onMenu == "log") {
          ...
          if (!Game.Has("Neuromancy")) {
            if (me.bought > 0 && me.hide != 3) {
              str2 +=
                ...
                Game.getTooltip(
                  '<div style="min-width:200px;"><div style="float:right;"><span class="price">' +
                    Beautify(Math.round(me.basePrice)) +
                    '</span></div><small>[Upgrade] [Purchased]</small><div class="name">' +
                    me.name +
            ...
            str2 +=
              '<div onclick="Game.UpgradesById[' +
              me.id +
              '].toggle();" class="crate upgrade' +
              (me.bought > 0 ? " enabled" : "") +
              '" ' +
              Game.getTooltip(
                '<div style="min-width:200px;"><div style="float:right;"><span class="price">' +
                  Beautify(Math.round(me.basePrice)) +
                  "</span></div><small>[Upgrade]" +
                  (me.bought > 0 ? " [Purchased]" : "") +
                  '</small><div class="name">' +
                  me.name +
                  '</div><div class="description">' +
                  me.desc +
                  "</div></div>",
        ...
        for (var i in list) {
          ...
          if (me.won > 0 && me.hide == 3) {
            shadowAchievements +=
              '<div class="crate achievement enabled" ' +
              Game.getTooltip(
                '<div style="min-width:200px;"><small>[Achievement] [Unlocked]' +
                  (me.hide == 3 ? " [Shadow]" : "") +
                  '</small><div class="name">' +
                  me.name +
                  '</div><div class="description">' +
            ...
            achievements +=
              '<div class="crate achievement" ' +
              Game.getTooltip(
                '<div style="min-width:200px;"><small>[Achievement]</small><div class="name">' +
                  me.name +
                  '</div><div class="description">' +
                  me.desc +
                  "</div></div>",
        ...
        str +=
          ...
          "</div></div>" +
          (Game.cookiesReset > 0
            ? '<div class="listing"><b>Cookies forfeited by resetting :</b> <div class="price plain">' +
              Beautify(Game.cookiesReset) +
              "</div></div>"
            : "") +
          '<div class="listing"><b>Game started :</b> ' +
          date +
          " ago</div>" +
          ...
          (Game.prestige["Heavenly chips"] > 0
            ? '</div><div class="subsection">' +
              ...
              '<div class="listing"><div class="icon" style="background-position:' +
              -19 * 48 +
              "px " +
              -7 * 48 +
              'px;"></div> <span style="vertical-align:100%;"><span class="title" style="font-size:22px;">' +
              Game.prestige["Heavenly chips"] +
              " heavenly chip" +
              (Game.prestige["Heavenly chips"] == 1 ? "" : "s") +
              "</span> (+" +
              Game.prestige["Heavenly chips"] * 2 +
              "% CpS)</span></div>"
            : "") +
          '</div><div class="subsection">' +
          '<div class="title">Upgrades unlocked</div>' +
          '<div class="listing"><b>Unlocked :</b> ' +
          upgradesOwned +
          "/" +
          upgradesTotal +
          " (" +
          Math.round((upgradesOwned / upgradesTotal) * 100) +
          "%)</div>" +
          ...
          achievementsTotal +
          " (" +
          Math.round((achievementsOwned / achievementsTotal) * 100) +
          "%)</div>" +
          '<div class="listing"><b>Milk :</b> ' +
          Math.round(Game.milkProgress * 100) +
          "% (" +
          milkName +
          ") <small>(Note : you gain milk through achievements. Milk can unlock unique upgrades over time.)</small></div>" +
          '<div class="listing" style="overflow-y:hidden;">' +
          achievements +
          "</div>" +
          (shadowAchievements != ""
    ...
    l("prefsButton").onclick = function () {
      Game.ShowMenu("prefs");
    };
    l("statsButton").onclick = function () {
      Game.ShowMenu("stats");
    };
    l("logButton").onclick = function () {
      Game.ShowMenu("log");
    };
    ...
    Game.getTooltip = function (text, x, y, origin) {
      ...
      return (
        'onMouseOut="Game.tooltip.hide();" onMouseOver="Game.tooltip.draw(this,\'' +
        escape(text) +
        "'," +
        x +
        "," +
    ...
    Game.getNewTicker = function () {
      ...
      if (Game.TickerN % 2 == 0 || Game.cookiesEarned >= 10100000000) {
          ...
          list.push(
            choose([
              "News : " +
                ...
                " miners dead in chocolate mine catastrophe!",
              "News : " +
                Math.floor(Math.random() * 1000 + 2) +
        ...
        if (Game.Has("Kitten overseers"))
          list.push(
            "News : locals report troups of bossy kittens meowing adorable orders at passerbys."
          );
    ...
    Game.TickerDraw = function () {
      ...
      if (Game.Ticker != "") {
        if (Game.TickerAge < Game.fps * 1 && 1 == 2) {
          //too bad this doesn't work well with html tags
          str = Game.Ticker.substring(0, (Game.Ticker + "<").indexOf("<"));
          str = str.substring(
            0,
            str.length * Math.min(1, Game.TickerAge / (Game.fps * 1))
          );
    ...
    Game.Object = function (
      ...
      this.setSpecial = function (
        what //change whether we're on the special overlay for this object or not
      ) {
      ...
      if (this.id != 0) {
        ...
        var str =
          ...
          this.id +
          '"><div class="separatorBottom"></div><div class="content"><div id="rowBackground' +
          this.id +
          '" class="background" style="background:url(img/' +
          this.background +
          '.png) repeat-x;"><div class="backgroundLeft"></div><div class="backgroundRight"></div></div><div class="objects" id="rowObjects' +
          this.id +
          '"> </div></div><div class="special" id="rowSpecial' +
          this.id +
          '"></div><div class="specialButton" id="rowSpecialButton' +
          this.id +
          '"></div><div class="info" id="rowInfo' +
          this.id +
          '"><div class="infoContent" id="rowInfoContent' +
          this.id +
          '"></div><div><a onclick="Game.ObjectsById[' +
          this.id +
          '].sell();">Sell 1</a></div></div></div>';
    ...
    Game.NewDrawFunction = function (
      ...
      return function () {
        ...
        for (var i = 0; i < this.amount; i++) {
          if (shift != 1) {
            var x =
              Math.floor(i / shift) * w +
              ((i % shift) / shift) * w +
              Math.floor((Math.random() - 0.5) * xVariance) +
              offX;
            var y =
              32 +
              heightOffset +
              Math.floor((Math.random() - 0.5) * yVariance) +
              (((-shift / 2) * 32) / 2 + ((i % shift) * 32) / 2) +
              offY;
          } else {
            var x =
              i * w + Math.floor((Math.random() - 0.5) * xVariance) + offX;
            var y =
              32 +
              heightOffset +
              Math.floor((Math.random() - 0.5) * yVariance) +
          ...
          str +=
            '<div class="object" style="background:url(img/' +
            usedPic +
            ".png);left:" +
            x +
            "px;top:" +
    ...
    Game.RebuildStore = function () //redraw the store from scratch
    {
      ...
      for (var i in Game.Objects) {
        ...
        str +=
          '<div class="product" ' +
          Game.getTooltip(
            '<div style="min-width:300px;"><div style="float:right;"><span class="price">' +
          ...
          me.icon +
          '.png);"></div><div class="content"><div class="title">' +
          me.displayName +
          '</div><span class="price">' +
          Beautify(Math.round(me.price)) +
          "</span>" +
          (me.amount > 0
            ? '<div class="title owned">' + me.amount + "</div>"
            : "") +
          "</div></div>";
      }
    ...
    new Game.Object(
      ...
      function () {
        ...
        return Game.ComputeCps(
          0.1,
          Game.Has("Reinforced index finger") * 0.1,
          Game.Has("Carpal tunnel prevention cream") + Game.Has("Ambidextrous"),
          add
        );
      },
      function () {
        ...
        if (!l("rowInfo" + this.id))
          l("sectionLeftInfo").innerHTML =
            '<div class="info" id="rowInfo' +
            this.id +
            '"><div class="infoContent" id="rowInfoContent' +
            this.id +
            '"></div><div><a onclick="Game.ObjectsById[' +
            this.id +
            '].sell();">Sell 1</a></div></div>';
      },
      function () {
        if (this.amount >= 1)
          Game.Unlock([
    ...
    new Game.Object(
      ...
      function () {
        ...
        return Game.ComputeCps(
          0.5,
          Game.Has("Forwards from grandma") * 0.3 + add,
          Game.Has("Steel-plated rolling pins") +
            Game.Has("Lubricated dentures") +
            Game.Has("Prune juice") +
            mult
        );
      },
      Game.NewDrawFunction(
        function () {
          var list = ["grandma"];
    ...
    new Game.Object(
      ...
      function () {
        return Game.ComputeCps(
          2,
          Game.Has("Cheap hoes") * 0.5,
          Game.Has("Fertilizer") +
            Game.Has("Cookie trees") +
            Game.Has("Genetically-modified cookies")
        );
      },
      Game.NewDrawFunction(0, 16, 16, 64, 2, 32),
      function () {
    ...
    new Game.Object(
      "Factory",
      "factory|factories|mass-produced",
      ...
      function () {
        return Game.ComputeCps(
          10,
          Game.Has("Sturdier conveyor belts") * 4,
          Game.Has("Child labor") +
            Game.Has("Sweatshop") +
            Game.Has("Radium reactors")
        );
      },
      Game.NewDrawFunction(0, 32, 2, 64, 1, -22),
      function () {
        if (this.amount >= 1)
    ...
    new Game.Object(
      ...
      function () {
        return Game.ComputeCps(
          40,
          Game.Has("Sugar gas") * 10,
          Game.Has("Megadrill") +
            Game.Has("Ultradrill") +
            Game.Has("Ultimadrill")
        );
      },
      Game.NewDrawFunction(0, 16, 16, 64, 2, 24),
      function () {
    ...
    new Game.Object(
      ...
      function () {
        return Game.ComputeCps(
          100,
          Game.Has("Vanilla nebulae") * 30,
          Game.Has("Wormholes") +
            Game.Has("Frequent flyer") +
            Game.Has("Warp drive")
        );
      },
      Game.NewDrawFunction(0, 16, 16, 64),
      function () {
    ...
    new Game.Object(
      "Alchemy lab",
      "alchemy lab|alchemy labs|transmuted",
      "Turns gold into cookies!",
      "alchemylab",
      "alchemylabIcon",
      "alchemylabBackground",
      200000,
      function () {
        return Game.ComputeCps(
          400,
          Game.Has("Antimony") * 100,
          Game.Has("Essence of dough") +
            Game.Has("True chocolate") +
            Game.Has("Ambrosia")
        );
      },
      Game.NewDrawFunction(0, 16, 16, 64, 2, 16),
      function () {
    ...
    new Game.Object(
      ...
      function () {
        return Game.ComputeCps(
          6666,
          Game.Has("Ancient tablet") * 1666,
          Game.Has("Insane oatling workers") +
            Game.Has("Soul bond") +
            Game.Has("Sanity dance")
        );
      },
      Game.NewDrawFunction(0, 32, 32, 64, 2),
      function () {
        if (this.amount >= 1)
    ...
    new Game.Object(
      "Time machine",
      "time machine|time machines|recovered",
      ...
      function () {
        return Game.ComputeCps(
          98765,
          Game.Has("Flux capacitors") * 9876,
          Game.Has("Time paradox resolver") +
            Game.Has("Quantum conundrum") +
            Game.Has("Causality enforcer")
        );
      },
      Game.NewDrawFunction(0, 32, 32, 64, 1),
      function () {
        if (this.amount >= 1)
    ...
    new Game.Object(
      ...
      "antimatter condenser|antimatter condensers|condensed",
      "Condenses the antimatter in the universe into cookies.",
      "antimattercondenser",
      "antimattercondenserIcon",
      "antimattercondenserBackground",
      3999999999,
      function () {
        return Game.ComputeCps(
          999999,
          Game.Has("Sugar bosons") * 99999,
          Game.Has("String theory") +
            Game.Has("Large macaron collider") +
            Game.Has("Big bang bake")
        );
      },
      Game.NewDrawFunction(0, 0, 64, 64, 1),
      function () {
    ...
    Game.Objects["Antimatter condenser"].displayName =
      '<span style="font-size:65%;">Antimatter condenser</span>'; //shrink the name since it's so large
      ...
      for (var i in Game.UpgradesInStore) {
        ...
        str +=
          ...
          Game.getTooltip(
            ...
            '<div style="min-width:200px;"><div style="float:right;"><span class="price">' +
              Beautify(Math.round(me.basePrice)) +
              '</span></div><small>[Upgrade]</small><div class="name">' +
              me.name +
              '</div><div class="description">' +
              me.desc +
              "</div></div>",
    ...
    new Game.Upgrade(
      "Reinforced index finger",
      "The mouse gains <b>+1</b> cookie per click.<br>Cursors gain <b>+0.1</b> base CpS.<q>prod prod</q>",
      100,
      [0, 0]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Billion fingers",
      "The mouse and cursors gain <b>+2</b> cookies for each non-cursor object owned.<q>clickityclickityclickity</q>",
      500000000,
      [2, 6]
    );
    new Game.Upgrade(
      "Trillion fingers",
      "The mouse and cursors gain <b>+10</b> cookies for each non-cursor object owned.<q>clickityclickityclickityclickity</q>",
      5000000000,
      [3, 6]
    );
    ...
    new Game.Upgrade(
      ...
      "Grandmas gain <b>+0.3</b> base CpS.<q>RE:RE:thought you'd get a kick out of this ;))</q>",
      Game.Objects["Grandma"].basePrice * tier1,
      [1, 0]
    );
    new Game.Upgrade(
      "Steel-plated rolling pins",
      "Grandmas are <b>twice</b> as efficient.",
      Game.Objects["Grandma"].basePrice * tier2,
      [1, 0]
    );
    new Game.Upgrade(
      "Lubricated dentures",
      "Grandmas are <b>twice</b> as efficient.<q>Squish</q>",
      Game.Objects["Grandma"].basePrice * tier3,
      [1, 1]
    );
    ...
    new Game.Upgrade(
      "Cheap hoes",
      "Farms gain <b>+0.5</b> base CpS.",
      Game.Objects["Farm"].basePrice * tier1,
      [2, 0]
    );
    new Game.Upgrade(
      "Fertilizer",
      "Farms are <b>twice</b> as efficient.<q>It's chocolate, I swear.</q>",
      Game.Objects["Farm"].basePrice * tier2,
      [2, 0]
    );
    new Game.Upgrade(
      "Cookie trees",
      "Farms are <b>twice</b> as efficient.<q>A relative of the breadfruit.</q>",
      Game.Objects["Farm"].basePrice * tier3,
      [2, 1]
    );
    ...
    new Game.Upgrade(
      ...
      "Factories gain <b>+4</b> base CpS.",
      Game.Objects["Factory"].basePrice * tier1,
      [4, 0]
    );
    new Game.Upgrade(
      "Child labor",
      "Factories are <b>twice</b> as efficient.<q>Cheaper, healthier workforce - and so much more receptive to whipping!</q>",
      Game.Objects["Factory"].basePrice * tier2,
      [4, 0]
    );
    new Game.Upgrade(
      "Sweatshop",
      "Factories are <b>twice</b> as efficient.<q>Slackers will be terminated.</q>",
      Game.Objects["Factory"].basePrice * tier3,
      [4, 1]
    );
    ...
    new Game.Upgrade(
      "Sugar gas",
      "Mines gain <b>+10</b> base CpS.<q>A pink, volatile gas, found in the depths of some chocolate caves.</q>",
      Game.Objects["Mine"].basePrice * tier1,
      [3, 0]
    );
    new Game.Upgrade(
      "Megadrill",
      "Mines are <b>twice</b> as efficient.",
      Game.Objects["Mine"].basePrice * tier2,
      [3, 0]
    );
    new Game.Upgrade(
      "Ultradrill",
      "Mines are <b>twice</b> as efficient.",
      Game.Objects["Mine"].basePrice * tier3,
      [3, 1]
    );
    ...
    new Game.Upgrade(
      "Antimony",
      "Alchemy labs gain <b>+100</b> base CpS.<q>Actually worth a lot of mony.</q>",
      Game.Objects["Alchemy lab"].basePrice * tier1,
      [6, 0]
    );
    new Game.Upgrade(
      "Essence of dough",
      "Alchemy labs are <b>twice</b> as efficient.<q>Extracted through the 5 ancient steps of alchemical baking.</q>",
      Game.Objects["Alchemy lab"].basePrice * tier2,
      [6, 0]
    );
    new Game.Upgrade(
      "True chocolate",
      "Alchemy labs are <b>twice</b> as efficient.<q>The purest form of cacao.</q>",
      Game.Objects["Alchemy lab"].basePrice * tier3,
      [6, 1]
    );
    ...
    new Game.Upgrade(
      "Ancient tablet",
      "Portals gain <b>+1,666</b> base CpS.<q>A strange slab of peanut brittle, holding an ancient cookie recipe. Neat!</q>",
      Game.Objects["Portal"].basePrice * tier1,
      [7, 0]
    );
    new Game.Upgrade(
      "Insane oatling workers",
      "Portals are <b>twice</b> as efficient.<q>ARISE, MY MINIONS!</q>",
      Game.Objects["Portal"].basePrice * tier2,
      [7, 0]
    );
    new Game.Upgrade(
      "Soul bond",
      "Portals are <b>twice</b> as efficient.<q>So I just sign up and get more cookies? Sure, whatever!</q>",
      Game.Objects["Portal"].basePrice * tier3,
      [7, 1]
    );
    ...
    new Game.Upgrade(
      "Flux capacitors",
      "Time machines gain <b>+9,876</b> base CpS.<q>Bake to the future.</q>",
      1234567890,
      [8, 0]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Quantum conundrum",
      "Time machines are <b>twice</b> as efficient.<q>It's full of stars!</q>",
      98765456789,
      [8, 1]
    );
    ...
    new Game.Upgrade(
      ...
      "Cookie production multiplier <b>+5%</b>.<q>No raisin to hate these.</q>",
      99999999,
      [0, 3]
    );
    new Game.Upgrade(
      "Peanut butter cookies",
      "Cookie production multiplier <b>+5%</b>.",
      99999999,
      [1, 3]
    );
    new Game.Upgrade(
      "Plain cookies",
      "Cookie production multiplier <b>+5%</b>.<q>Meh.</q>",
      99999999,
      [2, 3]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Double-chip cookies",
      "Cookie production multiplier <b>+10%</b>.",
      99999999999,
      [6, 3]
    );
    power = 5;
    ...
    new Game.Upgrade(
      "Quadrillion fingers",
      "The mouse and cursors gain <b>+20</b> cookies for each non-cursor object owned.<q>clickityclickityclickityclickityclick</q>",
      50000000000,
      [3, 6]
    );
    ...
    new Game.Upgrade(
      ...
      Game.Objects["Grandma"].basePrice * tier4,
      [1, 2]
    );
    order = 300;
    new Game.Upgrade(
      ...
      Game.Objects["Farm"].basePrice * tier4,
      [2, 2]
    );
    order = 400;
    new Game.Upgrade(
      ...
      Game.Objects["Factory"].basePrice * tier4,
      [4, 2]
    );
    order = 500;
    new Game.Upgrade(
      ...
      Game.Objects["Mine"].basePrice * tier4,
      [3, 2]
    );
    order = 600;
    new Game.Upgrade(
      ...
      Game.Objects["Shipment"].basePrice * tier4,
      [5, 2]
    );
    order = 700;
    new Game.Upgrade(
      ...
      Game.Objects["Alchemy lab"].basePrice * tier4,
      [6, 2]
    );
    order = 800;
    new Game.Upgrade(
      ...
      Game.Objects["Portal"].basePrice * tier4,
      [7, 2]
    );
    order = 900;
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Lucky day",
      "Golden cookies appear <b>twice as often</b> and last <b>twice as long</b>.",
      777777777,
      [10, 1]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      ...
      "Cookie production multiplier <b>+15%</b>.",
      999999999999,
      [10, 3]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Grandmas' grandmas",
      "Grandmas are <b>twice</b> as efficient.",
      Game.Objects["Time machine"].basePrice * tier2,
      [10, 9],
      function () {
        Game.Objects["Grandma"].drawFunction();
      }
    ...
    Game.SetResearch = function (what, time) {
      if (Game.Upgrades[what]) {
        Game.researchT = Game.Has("Ultrascience")
          ? Game.fps * 5
          : Game.baseResearchTime;
    ...
    new Game.Upgrade(
      ...
      "[Research]<br>Cookie production multiplier <b>+1%</b>.<q>Computer-designed chocolate chips. Computer chips, if you will.</q>",
      10000000000,
      [0, 9],
      function () {
    ...
    new Game.Upgrade(
      ...
      "[Research]<br>Cookie production multiplier <b>+2%</b>.<q>Now more aerodynamic than ever!</q>",
      20000000000,
      [1, 9],
      function () {
    ...
    new Game.Upgrade(
      "Ritual rolling pins",
      "[Research]<br>Grandmas are <b>twice</b> as efficient.<q>The result of years of scientific research!</q>",
      40000000000,
      [2, 9],
      function () {
    ...
    new Game.Upgrade(
      "Underworld ovens",
      "[Research]<br>Cookie production multiplier <b>+3%</b>.<q>Powered by science, of course!</q>",
      80000000000,
      [3, 9],
      function () {
    ...
    new Game.Upgrade(
      "One mind",
      '[Research]<br>Each grandma gains <b>+1 base CpS for each 50 grandmas</b>.<div class="warning">Note : the grandmothers are growing restless. Do not encourage them.</div><q>We are one. We are many.</q>',
      160000000000,
      [4, 9],
      function () {
        Game.elderWrath = 1;
    ...
    new Game.Upgrade(
      "Exotic nuts",
      "[Research]<br>Cookie production multiplier <b>+4%</b>.<q>You'll go crazy over these!</q>",
      320000000000,
      [5, 9],
      function () {
    ...
    new Game.Upgrade(
      "Communal brainsweep",
      '[Research]<br>Each grandma gains another <b>+1 base CpS for each 50 grandmas</b>.<div class="warning">Note : proceeding any further in scientific research may have unexpected results. You have been warned.</div><q>We fuse. We merge. We grow.</q>',
      640000000000,
      [6, 9],
      function () {
        Game.elderWrath = 2;
    ...
    new Game.Upgrade(
      "Arcane sugar",
      "[Research]<br>Cookie production multiplier <b>+5%</b>.<q>Tastes like insects, ligaments, and molasses.</q>",
      1280000000000,
      [7, 9],
      function () {
    ...
    new Game.Upgrade(
      "Elder Pact",
      '[Research]<br>Each grandma gains <b>+1 base CpS for each 20 portals</b>.<div class="warning">Note : this is a bad idea.</div><q>squirm crawl slither writhe<br>today we rise</q>',
      2560000000000,
      [8, 9],
      function () {
        Game.elderWrath = 3;
    ...
    new Game.Upgrade(
      "Elder Pledge",
      "[Repeatable]<br>Contains the wrath of the elders, at least for a while.",
      1,
      [9, 9],
      function () {
        Game.elderWrath = 0;
        Game.pledges++;
        Game.pledgeT =
          Game.fps * 60 * (Game.Has("Sacrificial rolling pins") ? 60 : 30);
        Game.Upgrades["Elder Pledge"].basePrice = Math.pow(
          8,
          Math.min(Game.pledges + 2, 13)
        );
    ...
    new Game.Upgrade(
      "Eclipse cookies",
      "Cookie production multiplier <b>+15%</b>.<q>Look to the cookie.</q>",
      9999999999999,
      [0, 4]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      "Quintillion fingers",
      "The mouse and cursors gain <b>+100</b> cookies for each non-cursor object owned.<q>man, just go click click click click click, it's real easy, man.</q>",
      50000000000000,
      [3, 6]
    );
    ...
    new Game.Upgrade(
      "Elder Covenant",
      "[Switch]<br>Puts a permanent end to the elders' wrath, at the price of 5% of your CpS.",
      6666666666666,
      [8, 9],
      function () {
        Game.pledgeT = 0;
    ...
    new Game.Upgrade(
      ...
      "[Switch]<br>You will get 5% of your CpS back, but the grandmatriarchs will return.",
      6666666666,
      [8, 9],
      function () {
    ...
    new Game.Upgrade(
      ...
      "Elder pledge last <b>twice</b> as long.",
      2888888888888,
      [2, 9]
    );
    ...
    new Game.Upgrade(
      "Stroopwafels",
      "Cookie production multiplier <b>+15%</b>.<q>If it ain't dutch, it ain't much.</q>",
      99999999999999,
      [3, 4]
    );
    new Game.Upgrade(
    ...
    new Game.Upgrade(
      ...
      "Cookie production multiplier <b>+15%</b>.<q>Quite.</q>",
      99999999999999,
      [11, 4]
    );
    type = "";
    ...
    new Game.Upgrade(
      "Sugar bosons",
      "Antimatter condensers gain <b>+99,999</b> base CpS.",
      Game.Objects["Antimatter condenser"].basePrice * tier1,
      [13, 0]
    );
    new Game.Upgrade(
      "String theory",
      "Antimatter condensers are <b>twice</b> as efficient.",
      Game.Objects["Antimatter condenser"].basePrice * tier2,
      [13, 0]
    );
    new Game.Upgrade(
      "Large macaron collider",
      "Antimatter condensers are <b>twice</b> as efficient.<q>How singular!</q>",
      Game.Objects["Antimatter condenser"].basePrice * tier3,
      [13, 1]
    );
    new Game.Upgrade(
      "Big bang bake",
      "Antimatter condensers are <b>twice</b> as efficient.<q>And that's how it all began.</q>",
      Game.Objects["Antimatter condenser"].basePrice * tier4,
      [13, 2]
    );
    ...
    new Game.Upgrade(
      "Madeleines",
      "Cookie production multiplier <b>+20%</b>.<q>Unforgettable!</q>",
      199999999999999,
      [12, 3]
    );
    new Game.Upgrade(
      "Palmiers",
      "Cookie production multiplier <b>+20%</b>.",
      199999999999999,
      [13, 3]
    );
    new Game.Upgrade(
    ...
    for (var i = 0; i < Game.moneyAchievs.length / 2; i++) {
      ...
      new Game.Achievement(
        Game.moneyAchievs[i * 2],
        "Bake <b>" +
          Beautify(Game.moneyAchievs[i * 2 + 1]) +
          "</b> cookie" +
          (Game.moneyAchievs[i * 2 + 1] == 1 ? "" : "s") +
          ".",
        pic,
        2
      );
    ...
    for (var i = 0; i < Game.cpsAchievs.length / 2; i++) {
      ...
      new Game.Achievement(
        Game.cpsAchievs[i * 2],
        "Bake <b>" +
          Beautify(Game.cpsAchievs[i * 2 + 1]) +
          "</b> cookie" +
          (Game.cpsAchievs[i * 2 + 1] == 1 ? "" : "s") +
          " per second.",
        pic,
        2
      );
    ...
    new Game.Achievement(
      "Sacrifice",
      "Reset your game with <b>1 million</b> cookies baked.<q>Easy come, easy go.</q>",
      [11, 6],
      2
    );
    new Game.Achievement(
      "Oblivion",
      "Reset your game with <b>1 billion</b> cookies baked.<q>Back to square one.</q>",
      [11, 6],
      2
    );
    new Game.Achievement(
      "From scratch",
      "Reset your game with <b>1 trillion</b> cookies baked.<q>It's been fun.</q>",
      [11, 6],
      2
    );
    ...
    new Game.Achievement(
      "Mathematician",
      "Have at least <b>1 time machine, 2 portals, 4 alchemy labs, 8 shipments</b> and so on (128 max).",
      [7, 6],
      2
    );
    new Game.Achievement(
      "Base 10",
      "Have at least <b>10 time machines, 20 portals, 30 alchemy labs, 40 shipments</b> and so on.",
      [8, 6],
      2
    );
    ...
    new Game.Achievement(
      "Nihilism",
      "Reset your game with <b>1 quadrillion</b> cookies baked.<q>There are many things<br>that need to be erased</q>",
      [11, 6],
      2
    );
      ...
      if (Math.abs(Game.elderWrath + 1 - Game.elderWrathD) > 0.1) {
        if (Game.elderWrathD < 1) {
          ...
          if (Game.bg != Game.defaultBg || Game.bgFade != Game.defaultBg) {
            ...
            l("backgroundLayer1").style.background =
              "url(img/" + Game.bg + ".jpg)";
            l("backgroundLayer2").style.background =
              "url(img/" + Game.bgFade + ".jpg)";
  ...
  Game.Logic = function () {
    ...
    if (
      ...
      Game.cookieParticleAdd(); //cookie shower
    if (Game.frenzy > 0) {
      Game.frenzy--;
    ...
    if (Game.T % (Game.fps * 5) == 0 && Game.ObjectsById.length > 0) {
      ...
      if (Game.cookiesEarned >= 99999999)
        Game.Unlock([
          "Coconut cookies",
          "White chocolate cookies",
          "Macadamia nut cookies",
        ]);
      if (Game.cookiesEarned >= 999999999)
        Game.Unlock([
          "Double-chip cookies",
          "White chocolate macadamia nut cookies",
          "All-chocolate cookies",
        ]);
      if (Game.cookiesEarned >= 9999999999)
        Game.Unlock([
          "Dark chocolate-coated cookies",
          "White chocolate-coated cookies",
        ]);
      if (Game.cookiesEarned >= 99999999999)
        Game.Unlock(["Eclipse cookies", "Zebra cookies"]);
      if (Game.cookiesEarned >= 999999999999)
        Game.Unlock(["Snickerdoodles", "Stroopwafels", "Macaroons"]);
      if (
        Game.cookiesEarned >= 999999999999 &&
        Game.Has("Snickerdoodles") &&
        Game.Has("Stroopwafels") &&
        Game.Has("Macaroons")
      ) {
        Game.Unlock("Empire biscuits");
        if (Game.Has("Empire biscuits")) Game.Unlock("British tea biscuits");
        if (Game.Has("British tea biscuits"))
          Game.Unlock("Chocolate british tea biscuits");
        if (Game.Has("Chocolate british tea biscuits"))
          Game.Unlock("Round british tea biscuits");
        if (Game.Has("Round british tea biscuits"))
          Game.Unlock("Round chocolate british tea biscuits");
        if (Game.Has("Round chocolate british tea biscuits"))
          Game.Unlock("Round british tea biscuits with heart motif");
        if (Game.Has("Round british tea biscuits with heart motif"))
          Game.Unlock("Round chocolate british tea biscuits with heart motif");
      }
      if (Game.cookiesEarned >= 9999999999999)
        Game.Unlock(["Madeleines", "Palmiers", "Palets", "Sabl&eacute;s"]);

      for (var i = 0; i < Game.moneyAchievs.length / 2; i++) {
        if (Game.cookiesEarned >= Game.moneyAchievs[i * 2 + 1])
          Game.Win(Game.moneyAchievs[i * 2]);
      }
      var buildingsOwned = 0;
      var oneOfEach = 1;
      var mathematician = 1;
      ...
      if (
        ...
        Game.Has("Farmer grandmas") &&
        Game.Has("Worker grandmas") &&
        Game.Has("Miner grandmas") &&
        Game.Has("Cosmic grandmas") &&
        Game.Has("Transmuted grandmas") &&
        Game.Has("Altered grandmas") &&
        Game.Has("Grandmas' grandmas")
      )
        Game.Win("Elder");
      if (
      ...
      if (
        ...
        l("bigCookie").getBoundingClientRect().bottom >
          l("milk").getBoundingClientRect().top + 16 &&
  ...
  Game.Draw = function () {
    ...
    if (Game.prefs.milk) {
      var x = Math.floor(
        (Game.T * 2 +
          Math.sin(Game.T * 0.1) * 2 +
          Math.sin(Game.T * 0.03) * 2 -
          (Game.milkH - Game.milkHd) * 2000) %
          480
      );
      var y = 0;
      var m1 = l("milkLayer1");
      ...
      if (Game.milkProgress < 1) {
        m1o = 1;
        m1i = "milkWave";
        m2i = "chocolateMilkWave";
      } else if (Game.milkProgress < 2) {
        m1o = 1 - (Game.milkProgress - 1);
        m1i = "milkWave";
        m2i = "chocolateMilkWave";
      } else if (Game.milkProgress < 3) {
        m1o = 1 - (Game.milkProgress - 2);
      ...
      m2o = 1 - m1o;
      if (m1.style.backgroundImage != "url(img/" + m1i + ".png")
        m1.style.backgroundImage = "url(img/" + m1i + ".png)";
      if (m2.style.backgroundImage != "url(img/" + m2i + ".png")
        m2.style.backgroundImage = "url(img/" + m2i + ".png)";
    ...
    if (Game.prefs.particles) {
      if (Game.elderWrathD <= 1.5) {
        if (Game.cookiesPs >= 1000)
          l("cookieShower").style.backgroundImage =
            "url(img/cookieShower3.png)";
        else if (Game.cookiesPs >= 500)
          l("cookieShower").style.backgroundImage =
            "url(img/cookieShower2.png)";
        else if (Game.cookiesPs >= 50)
          l("cookieShower").style.backgroundImage =
            "url(img/cookieShower1.png)";
        else l("cookieShower").style.backgroundImage = "none";
        l("cookieShower").style.backgroundPosition =
          "0px " + (Math.floor(Game.T * 2) % 512) + "px";
      }
      if (Game.elderWrathD >= 1 && Game.elderWrathD < 1.5)
        l("cookieShower").style.opacity = 1 - (Game.elderWrathD - 1) / 0.5;
    }

    var unit = Math.round(Game.cookiesd) == 1 ? " cookie" : " cookies";
    if (Math.round(Game.cookiesd).toString().length > 11) unit = "<br>cookies";
    l("cookies").innerHTML =
      Beautify(Math.round(Game.cookiesd)) +
      unit +
      '<div style="font-size:50%;">per second : ' +
      Beautify(Game.cookiesPs, 1) +
    ...
    for (var i in Game.Objects) {
      ...
      if (Game.cookies >= me.price)
        l("product" + me.id).className = "product enabled";
      else l("product" + me.id).className = "product disabled";

      //update object info
      if (l("rowInfo" + me.id) && Game.T % 5 == 0)
        l("rowInfoContent" + me.id).innerHTML =
          "&bull; " +
          me.amount +
          " " +
          (me.amount == 1 ? me.single : me.plural) +
          "<br>&bull; producing " +
          Beautify(me.storedTotalCps, 1) +
          " " +
          (me.storedTotalCps == 1 ? "cookie" : "cookies") +
          " per second<br>&bull; total : " +
    ...
    for (var i in Game.UpgradesInStore) {
      var me = Game.UpgradesInStore[i];
      if (Game.cookies >= me.basePrice)
        l("upgrade" + i).className = "crate upgrade enabled";
